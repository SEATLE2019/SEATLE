...
parser.add_argument('--city', type=str, default='Tor', help='city')
...
parser.add_argument('--obv', type=int, default=10, help='Every obv we output the performance on training set')
...
parser.add_argument('--num_query_per_cls', type=int, default=8, help='number of query instances for each business')

parser.add_argument('--feature_dim', type=int, default=10, help='the feature dimension for an instance after MLP')
parser.add_argument('--user_feature_dim', type=int, default=10, help='the feature dimension to represent a user')
parser.add_argument('--loc_feature_dim', type=int, default=10, help='the feature dimension to represent a loc')
parser.add_argument('--context_vector_size', type=int, default=10,
                    help='the size of the context vector in the reference attention')

parser.add_argument('--num_heads', type=int, default=1, help='number of heads in the multihead attention')

parser.add_argument('--distance_threshold', type=float, default=0.01, help='the distance threshold')
...
def test(algo):
    ...
    '''
    print('Loading and creating friends graph data...')
    friends_file = 'data/' + args.city + '/data/'  + args.city + '_friends.txt'
    friends_adjacent_matrix, user_num = load_friends_data(friends_file)
    '''
    ...
    alg = MetaLearningWrapper(learning_rate=args.learning_rate, keep_rate=1.0, margin=args.margin,
                              obv=args.obv, num_query_per_cls=n_users,#num_query_per_cls=args.num_query_per_cls,
    ...
    for epi_cls in range(nb_classes):
        ...
        user_num = len(pos_vec[0][0])

        infer_start = 0
        for start in range(user_num / args.num_query_per_cls):
            curr_pos_vec = [pos_vec[0][infer_start:infer_start + args.num_query_per_cls]]
            infer_start += args.num_query_per_cls
            vali_loss, vali_acc, pos_scores, neg_scores = algo.evaluate(reference_vec, curr_pos_vec, curr_pos_vec)
            for each_score in pos_scores:
                Str_IOString.write(str(each_score) + '\n')
        if epi_cls % 100 == 0 and epi_cls > 0:
...
def train():
    ...
    '''
    print('Loading and creating friends graph data...')
    friends_file = 'data/' + args.city + '_friends.txt'
    friends_adjacent_matrix, user_num = load_friends_data(friends_file)
    '''
    ...
    nb_classes = len(pos_train_dict)  # it is equal to the number of busineses
    n_reference = args.n_reference
    n_chosen_business = min(args.n_chosen_business, nb_classes);  # number of classes in each episode
    ...
    print 'num query for class is: ', n_query
    ...
    # by doing this, we increase the sampling prob for businesses with less check-ins
    ...
    for each_epoch in range(args.n_epochs):
        ...
        for each_episode in range(n_episodes):
            ...
            # sample classes based on their check-in number
...
def visualize():
    ...
    n_reference = args.n_reference
    alg = MetaLearningWrapper(learning_rate=args.learning_rate, keep_rate=1.0, margin=args.margin,
                              obv=args.obv, num_query_per_cls=21,
                              num_ref_per_cls=n_reference, user_feature_dim=args.user_feature_dim,
                              context_vector_size = args.context_vector_size, num_heads=args.num_heads,
                              loc_feature_dim=args.loc_feature_dim, feature_dim=args.feature_dim,
                              loc_ADJ=location_adjacent_matrix, loc_num=loc_num, user_num=user_num)
    ...
    for i in range(500):
        size = len(pos_train_dict[i])
        if size > 20 and size < 40:
            print i, size
    ...
    fout = open('/Users/rli02/Desktop/data_plot/ruirui/Tor_270.txt', 'w')
